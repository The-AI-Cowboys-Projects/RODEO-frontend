...
const StatCard = ({ title, value, subtitle, icon: Icon, trend, color = 'blue', isDarkMode }) => {
  const colorClasses = {
    blue: 'bg-blue-500',
    green: 'bg-green-500',
  ...
  return (
    <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
...
const RiskBadge = ({ level, isDarkMode }) => {
  const colors = isDarkMode ? {
    critical: 'bg-red-500/15 text-red-300',
    high: 'bg-orange-500/15 text-orange-300',
    medium: 'bg-yellow-500/15 text-yellow-300',
    low: 'bg-blue-500/15 text-blue-300',
    minimal: 'bg-green-500/15 text-green-300'
  } : {
...
const ComplianceBar = ({ score, label, isDarkMode }) => {
  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-1">
        ...
        <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{score}%</span>
      </div>
      <div className={`w-full rounded-full h-2.5 ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`}>
        <div
...
const CIIPDashboard = () => {
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <h3 className={`mt-2 text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>No data available</h3>
          <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Unable to load dashboard data</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900' : 'bg-gray-50'} p-6`}>
      ...
      {/* Overview Tab */}
      {activeTab === 'overview' && (
        <OverviewTab data={dashboardData} isDarkMode={isDarkMode} />
      )}

      {/* Compliance Tab */}
      {activeTab === 'compliance' && (
        <ComplianceTab isDarkMode={isDarkMode} />
      )}

      {/* Sectors Tab */}
      {activeTab === 'sectors' && (
        <SectorsTab isDarkMode={isDarkMode} />
      )}

      {/* Assets Tab */}
      {activeTab === 'assets' && (
        <AssetsTab isDarkMode={isDarkMode} />
      )}

      {/* Incidents Tab */}
      {activeTab === 'incidents' && (
        <IncidentsTab isDarkMode={isDarkMode} />
      )}
    </div>
  );
};
...
const OverviewTab = ({ data, isDarkMode }) => {
  const sectorIcons = {
    energy: BoltIcon,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
          ...
          <div className="space-y-4">
            {data.sector_distribution.map((sector) => {
              ...
              return (
                <div key={sector.sector} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  <div className="flex items-center space-x-3">
                    <Icon className="w-6 h-6 text-blue-600" />
                    <div>
                      <p className={`font-medium capitalize ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.sector}</p>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{sector.assets} assets</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Risk: {sector.risk_score}</p>
                    <RiskBadge level={sector.risk_score > 80 ? 'critical' : sector.risk_score > 70 ? 'high' : 'medium'} isDarkMode={isDarkMode} />
                  </div>
                </div>
              );
            })}
          ...
          <div className="space-y-3">
            {Object.entries(data.geographic_distribution).map(([region, count]) => (
              <div key={region} className="flex items-center justify-between">
                <span className={`text-sm font-medium uppercase ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{region}</span>
                <div className="flex items-center space-x-2">
                  <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{count} assets</span>
                  <div className={`w-32 rounded-full h-2 ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`}>
                    <div
      ...
      {/* Recent Activity */}
      <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
        <h2 className={`text-lg font-semibold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Recent Incidents</h2>
...
const ComplianceTab = ({ isDarkMode }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            ...
            <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Overall Score</p>
          </div>
        </div>
      </div>
      ...
      {Object.entries(complianceData.pillars).map(([pillar, details]) => (
        <div key={pillar} className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-lg font-semibold capitalize ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
              {pillar.replace(/_/g, ' ')}
            </h3>
            <div className="flex items-center space-x-3">
              <span className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{details.score}%</span>
          ...
          <div className="space-y-4">
            {Object.entries(details.requirements).map(([req, reqDetails]) => (
              <div key={req} className={`border-l-4 pl-4 ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {reqDetails.evidence && reqDetails.evidence.length > 0 && (
                      <div className="mt-2">
                        <p className={`text-xs font-medium mb-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Evidence:</p>
                  ...
                  <span className={`ml-4 px-2 py-1 rounded text-xs font-medium ${
                    reqDetails.status === 'compliant'
                      ? isDarkMode ? 'bg-green-500/15 text-green-300' : 'bg-green-100 text-green-800'
                      : reqDetails.status === 'partial'
                      ? isDarkMode ? 'bg-yellow-500/15 text-yellow-300' : 'bg-yellow-100 text-yellow-800'
                      : isDarkMode ? 'bg-red-500/15 text-red-300' : 'bg-red-100 text-red-800'
                  }`}>
                    {reqDetails.status}
          ...
          {details.recommendations && details.recommendations.length > 0 && (
            <div className={`mt-4 p-4 rounded-lg ${isDarkMode ? 'bg-blue-500/10' : 'bg-blue-50'}`}>
              <p className={`text-sm font-medium mb-2 ${isDarkMode ? 'text-blue-300' : 'text-blue-900'}`}>Recommendations:</p>
              <ul className={`list-disc list-inside text-sm space-y-1 ${isDarkMode ? 'text-blue-300' : 'text-blue-800'}`}>
      ...
      <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
        ...
        {['high_priority', 'medium_priority', 'low_priority'].map((priority) => (
          <div key={priority} className="mb-6">
            <h4 className={`text-sm font-medium mb-3 uppercase ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className="space-y-2">
              {complianceData.gap_analysis[priority].map((gap, idx) => (
                <div key={idx} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  <div className="flex-1">
                    <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{gap.gap}</p>
                    <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Pillar: {gap.pillar}</p>
                  </div>
                  <span className={`ml-4 px-3 py-1 rounded-full text-xs font-medium ${isDarkMode ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-100 text-blue-800'}`}>
                    {gap.target_date}
                  </span>
                </div>
              ))}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {sectors.map((sector) => {
        ...
        return (
          <div key={sector.id} className={`rounded-lg p-6 transition-shadow ${isDarkMode ? 'bg-slate-800 border border-slate-700 hover:border-purple-500/50' : 'bg-white shadow hover:shadow-lg'}`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-lg ${isDarkMode ? 'bg-blue-500/15' : 'bg-blue-100'}`}>
                  <Icon className="w-6 h-6 text-blue-600" />
                </div>
                <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.name}</h3>
              </div>
              <RiskBadge level={sector.risk_score > 80 ? 'critical' : sector.risk_score > 70 ? 'high' : 'medium'} isDarkMode={isDarkMode} />
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Assets</span>
                <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.assets}</span>
              </div>
              <div className="flex justify-between">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Crown Jewels</span>
                <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.crown_jewels}</span>
              </div>
              <div className="flex justify-between">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Risk Score</span>
                <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.risk_score}</span>
              </div>
              <div className="flex justify-between">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Incidents (30d)</span>
                <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.incidents_30d}</span>
              </div>
              <div className="flex justify-between">
                <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Compliance</span>
                <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{sector.compliance}%</span>
              </div>
            </div>

            <div className={`mt-4 pt-4 border-t ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
              <p className={`text-xs mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Key Systems:</p>
              <div className="flex flex-wrap gap-2">
                {sector.key_systems.map((system) => (
                  <span key={system} className={`px-2 py-1 rounded text-xs ${isDarkMode ? 'bg-slate-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
                    {system}
                  </span>
                ))}
              </div>
      ...
      {selectedSector && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={`rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
            {/* Modal Header */}
            <div className={`flex items-center justify-between p-6 border-b sticky top-0 z-10 ${isDarkMode ? 'border-slate-600 bg-slate-800' : 'border-gray-200 bg-white'}`}>
              <div className="flex items-center space-x-3">
                {(() => {
                ...
                <div>
                  <h2 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedSector.name} Sector</h2>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Critical Infrastructure Details</p>
                </div>
              </div>
              <button
              ...
              >
                <XMarkIcon className={`w-6 h-6 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
              </button>
            </div>
            ...
            {loadingDetails ? (
              <div className="p-8 text-center">
                ...
                <p className={`mt-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading sector details...</p>
              </div>
            ) : (
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
                    <p className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedSector.assets}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Assets</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-purple-900/20' : 'bg-purple-50'}`}>
                    <p className={`text-3xl font-bold ${isDarkMode ? 'text-purple-300' : 'text-purple-900'}`}>{selectedSector.crown_jewels}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-purple-300' : 'text-purple-600'}`}>Crown Jewels</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-red-900/20' : 'bg-red-50'}`}>
                    <p className={`text-3xl font-bold ${isDarkMode ? 'text-red-300' : 'text-red-900'}`}>{selectedSector.risk_score}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-red-300' : 'text-red-600'}`}>Risk Score</p>
                  </div>
                  <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-orange-900/20' : 'bg-orange-50'}`}>
                    <p className={`text-3xl font-bold ${isDarkMode ? 'text-orange-300' : 'text-orange-900'}`}>{selectedSector.incidents_30d}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-orange-300' : 'text-orange-600'}`}>Incidents (30d)</p>
                  </div>
                </div>
                ...
                <div className={`rounded-lg p-4 ${isDarkMode ? 'bg-slate-700' : 'bg-gray-50'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Compliance Score</h3>
                ...
                {/* Key Systems */}
                <div>
                  <h3 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Key Systems</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {selectedSector.key_systems.map((system) => (
                      <div key={system} className={`flex items-center space-x-2 p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-blue-50'}`}>
                        <ServerIcon className="w-5 h-5 text-blue-600" />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-blue-900'}`}>{system}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Risk Assessment */}
                <div>
                  <h3 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Risk Assessment</h3>
                  <div className="space-y-2">
                    <div className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                      <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Threat Level</span>
                      <RiskBadge level={selectedSector.risk_score > 80 ? 'critical' : selectedSector.risk_score > 70 ? 'high' : 'medium'} isDarkMode={isDarkMode} />
                    </div>
                    <div className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                      <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Protection Status</span>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        selectedSector.compliance >= 80
                          ? isDarkMode ? 'bg-green-500/15 text-green-300' : 'bg-green-100 text-green-800'
                          : selectedSector.compliance >= 60
                          ? isDarkMode ? 'bg-yellow-500/15 text-yellow-300' : 'bg-yellow-100 text-yellow-800'
                          : isDarkMode ? 'bg-red-500/15 text-red-300' : 'bg-red-100 text-red-800'
                      }`}>
                    ...
                    <div className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                      <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Incident Trend</span>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        selectedSector.incidents_30d <= 5
                          ? isDarkMode ? 'bg-green-500/15 text-green-300' : 'bg-green-100 text-green-800'
                          : selectedSector.incidents_30d <= 15
                          ? isDarkMode ? 'bg-yellow-500/15 text-yellow-300' : 'bg-yellow-100 text-yellow-800'
                          : isDarkMode ? 'bg-red-500/15 text-red-300' : 'bg-red-100 text-red-800'
                      }`}>
                ...
                {/* Actions */}
                <div className={`flex gap-3 pt-4 border-t ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                  <button
                    onClick={() => {
                  ...
                  <button
                    onClick={closeSectorModal}
                    className={`px-6 py-2 rounded-lg transition-colors font-medium ${isDarkMode ? 'bg-slate-700 text-gray-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                  >
                    Close
                  </button>
                </div>
...
const AssetsTab = ({ isDarkMode }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Filters */}
      <div className={`rounded-lg p-4 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <select
            value={filters.sector || ''}
            onChange={(e) => setFilters({...filters, sector: e.target.value || null})}
            className={`px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'border-gray-300'}`}
          >
      ...
      <div className="space-y-4">
        {assets.map((asset) => (
          <div key={asset.asset_id} className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{asset.name}</h3>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                  ...
                  <RiskBadge level={asset.risk_level} isDarkMode={isDarkMode} />
                </div>
              ...
              <div className="text-right">
                <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{asset.risk_score}</div>
                <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Risk Score</div>
              </div>
            </div>
      ...
      {selectedAsset && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={`rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
            ...
            <div className={`flex items-center justify-between p-6 border-b sticky top-0 z-10 ${isDarkMode ? 'border-slate-600 bg-slate-800' : 'border-gray-200 bg-white'}`}>
              <div>
                <h2 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedAsset.name}</h2>
                <div className="flex items-center space-x-2 mt-1">
                  <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{selectedAsset.asset_id}</span>
                  <span className={isDarkMode ? 'text-slate-500' : 'text-gray-300'}>|</span>
                  <span className={`text-sm uppercase ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{selectedAsset.sector}</span>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>IP Address</p>
                  <p className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedAsset.ip_address}</p>
                </div>
                <div className={`p-4 rounded-lg text-center ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Location</p>
                  <p className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedAsset.location}</p>
                </div>
              ...
              {/* Vulnerabilities */}
              <div>
                <h3 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Vulnerabilities</h3>
              ...
              {/* Security Controls */}
              <div>
                <h3 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Security Controls</h3>
              ...
              <div className={`rounded-lg p-4 ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                <div className="flex items-center justify-between">
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Compliance Status</h3>
              ...
              <div className={`rounded-lg p-4 ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                <div className="flex items-center justify-between">
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Risk Level</h3>
                  <RiskBadge level={selectedAsset.risk_level} isDarkMode={isDarkMode} />
                </div>
              </div>
              ...
              <div className={`flex gap-3 pt-4 border-t ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
                <button
                  onClick={() => setSelectedAsset(null)}
                  className={`flex-1 px-4 py-2 rounded-lg transition-colors font-medium ${isDarkMode ? 'bg-slate-700 text-gray-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                >
                  Close
                </button>
              </div>
  ...
  return (
    <div className="space-y-4">
      {incidents.map((incident) => (
        <div key={incident.incident_id} className={`rounded-lg p-6 ${isDarkMode ? 'bg-slate-800 border border-slate-700' : 'bg-white shadow'}`}>
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-2">
                <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{incident.incident_id}</h3>
                <span className={`px-2 py-1 rounded text-xs font-medium ${
              ...
              <p className={`mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{incident.description}</p>
              <div className={`flex items-center space-x-4 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <p className={`text-xs mb-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Attack Stage</p>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{incident.attack_stage.replace('_', ' ').toUpperCase()}</p>
            </div>
            <div>
              <p className={`text-xs mb-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Assigned To</p>
              <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{incident.assigned_to}</p>
            </div>
          </div>

          <div className="flex flex-wrap gap-2 mb-4">
            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>MITRE ATT&CK:</span>
            {incident.mitre_techniques.map((technique) => (
              <span key={technique} className={`px-2 py-1 rounded text-xs font-medium ${isDarkMode ? 'bg-purple-500/15 text-purple-300' : 'bg-purple-100 text-purple-800'}`}>
                {technique}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Response Actions:</span>
            {incident.response_actions.map((action) => (
              <span key={action} className={`px-2 py-1 rounded text-xs font-medium ${isDarkMode ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-100 text-blue-800'}`}>
                {action}
              </span>
            ))}
          </div>
