  ...
  return (
    <div className="p-6 space-y-6 text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Total Devices" value={totalDevices} color="text-white" />
        <StatCard label="Online" value={online} color="text-green-400" />
        <StatCard label="Alarm / Error" value={alarm} color="text-red-400" />
        <StatCard label="Offline / Degraded" value={offline} color="text-gray-400" />
      </div>

      {/* Device status grid */}
      <Section title="Device Status">
        {devices.length === 0 ? (
          <p className="text-gray-400 text-sm">No devices found. Start a scenario in ICS/SCADA to register devices.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {devices.map((d) => (
              <div
                key={d.device_id}
                className="rounded-lg border border-amber-500/20 bg-gray-900/60 p-4 hover:border-amber-500/50 transition-colors"
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold text-sm text-white truncate pr-2">{d.name}</span>
                  <span
                ...
                <div className="text-xs text-gray-400 space-y-0.5">
                  ...
                  <div>
                    ...
                    <span className={d.status === 'online' ? 'text-green-400' : d.status === 'alarm' || d.status === 'error' ? 'text-red-400' : 'text-gray-300'}>
                      {d.status}
                    </span>
                  </div>
                  {d.last_polled && (
      ...
      {/* Purdue Zone Diagram */}
      <Section title="Purdue Reference Model — Device Distribution">
            ...
            return (
              ...
              >
                ...
                <div className="flex-shrink-0 text-right">
                  <span className={`text-lg font-bold ${count > 0 ? 'text-amber-300' : 'text-gray-600'}`}>
                    {count}
                  </span>
                  <p className="text-xs text-gray-500">device{count !== 1 ? 's' : ''}</p>
                </div>
              </div>
            )
          })}
      ...
      {/* Alert Feed */}
      <Section title="Recent ICS Events">
        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {events.length === 0 ? (
            ...
            events.map((ev, i) => (
              <div
                key={ev.id || i}
                className="rounded-lg border border-gray-700 bg-gray-900/50 p-3 flex items-start gap-3"
              >
                <span
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white">{ev.event_type}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{ev.description || ev.data?.description || ''}</p>
        ...
        {!compliance ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {(compliance.frameworks || []).map((fw) => {
              ...
              return (
                <div key={fw.name} className="rounded-lg border border-amber-500/20 bg-gray-900/60 p-4">
                  ...
                  <div className="h-2 rounded-full bg-gray-800 overflow-hidden flex mb-3">
                  ...
                  <div className="flex gap-3 text-xs">
                    ...
                    <span className="text-gray-500 ml-auto">{fw.total} total</span>
                  </div>
                </div>
              )
            })}
