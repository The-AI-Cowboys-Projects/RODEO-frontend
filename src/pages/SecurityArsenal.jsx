          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center space-x-3 sm:space-x-4">
              <div className="w-12 h-12 sm:w-14 lg:w-16 sm:h-14 lg:h-16 bg-gradient-to-br from-red-500/40 to-orange-500/40 rounded-xl flex items-center justify-center border-2 border-red-400/60 shadow-lg shadow-red-500/40 animate-pulse flex-shrink-0">
                <ViewfinderCircleIcon className="w-6 h-6 sm:w-7 lg:w-9 sm:h-7 lg:h-9 text-red-300 drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
              </div>
              <div>
                ...
                <p className="mt-1 sm:mt-2 text-xs sm:text-sm lg:text-lg font-medium text-red-200">
      ...
      <div className={`${cardBg} rounded-xl border-2 shadow-xl p-4 sm:p-6 mb-6`}>
        ...
        {scanStatus && (
          <div className={`mt-5 p-4 rounded-xl flex items-center gap-3 border-2 ${
            scanStatus.status === 'running' ? 'bg-blue-900/30 border-blue-500/40 text-blue-300' :
            scanStatus.status === 'completed' ? 'bg-green-900/30 border-green-500/40 text-green-300' :
          ...
          <div className={`mt-5 pt-5 border-t-2 ${isDarkMode ? 'border-slate-600/50' : 'border-gray-200'}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {detectedTech.map((tech, idx) => (
                <span
                  key={idx}
                  className="px-4 py-2 text-base font-bold bg-cyan-500/20 text-cyan-300 rounded-lg border border-cyan-500/40"
                >
                  {tech.name}
        ...
        {[
          ...
          >
            ...
            {tab.id === 'findings' && vulnerabilities.length > 0 && (
              <span className="ml-2 px-2 sm:px-3 py-0.5 sm:py-1 text-xs sm:text-sm font-bold bg-red-500/30 text-red-300 rounded-lg border border-red-500/40">
                ...
                <div className={`${cardBg} rounded-xl shadow-xl border-2 p-5`}>
                  ...
                  <div className="space-y-3">
                    {recommendations.map((rec, idx) => (
                      <div
                        key={idx}
                        className="p-4 bg-cyan-500/10 rounded-lg border border-cyan-500/30"
                      >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className={`${isDarkMode ? 'bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-sm' : 'bg-white'} rounded-xl border-2 border-orange-500/30 shadow-xl p-5`}>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-bold text-orange-300 uppercase tracking-wider mb-2">
                      Scan Type
                    </label>
                    <select
                    ...
                    {realScanResults.type === 'trivy' ? (
                      <>
                        <div className="bg-red-900/30 rounded-lg p-3 border border-red-500/30">
                          <div className="text-2xl font-bold text-red-400">{realScanResults.summary.critical || 0}</div>
                        ...
                        <div className="bg-orange-900/30 rounded-lg p-3 border border-orange-500/30">
                          <div className="text-2xl font-bold text-orange-400">{realScanResults.summary.high || 0}</div>
                        ...
                        <div className="bg-yellow-900/30 rounded-lg p-3 border border-yellow-500/30">
                          <div className="text-2xl font-bold text-yellow-400">{realScanResults.summary.medium || 0}</div>
                          <div className="text-xs text-yellow-300 uppercase">Medium</div>
                        </div>
                        <div className="bg-blue-900/30 rounded-lg p-3 border border-blue-500/30">
                          <div className="text-2xl font-bold text-blue-400">{realScanResults.summary.low || 0}</div>
                          <div className="text-xs text-blue-300 uppercase">Low</div>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="bg-yellow-900/30 rounded-lg p-3 border border-yellow-500/30">
                          ...
                          <div className="text-xs text-yellow-300 uppercase">Filtered</div>
                        </div>
                        <div className="bg-cyan-900/30 rounded-lg p-3 border border-cyan-500/30">
                          <div className="text-2xl font-bold text-cyan-400">{realScanResults.summary.total_scanned || 0}</div>
                          <div className="text-xs text-cyan-300 uppercase">Total Scanned</div>
                        </div>
                      </>
                    )}
                  </div>
                    ...
                    <div className="space-y-2">
                      {realScanResults.results.vulnerabilities.slice(0, 20).map((vuln, idx) => (
                        <div key={idx} className={`p-3 rounded-lg border ${
                          ...
                          vuln.severity === 'HIGH' ? 'bg-orange-900/20 border-orange-500/30' :
                          vuln.severity === 'MEDIUM' ? 'bg-yellow-900/20 border-yellow-500/30' :
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <span className={`ml-2 px-2 py-0.5 text-xs rounded font-bold ${
                                ...
                                vuln.severity === 'MEDIUM' ? 'bg-yellow-500/30 text-yellow-300' :
